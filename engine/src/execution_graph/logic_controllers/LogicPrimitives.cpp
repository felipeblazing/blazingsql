...
namespace cache{
 ...
 void CacheMachine::finish(){
  ...
  CacheMachine::CacheMachine(unsigned long long gpuMemory, std::vector<unsigned long long> memoryPerCache, std::vector<CacheDataType> cachePolicyTypes)
  : _finished(false){
    cache.resize(cachePolicyTypes.size() + 1);
    memoryPerCache.push_back(gpuMemory);
    for( auto mem : memoryPerCache){
        memoryPerCache.push_back(mem);
    }

    usedMemory.resize(cache.size(),0UL);
    this->cachePolicyTypes.push_back(GPU);
    for(auto policy : cachePolicyTypes){
