...
from pyblazing.apiv2.comms import PollingPlugin
...
def initializeBlazing(
    ...
    filename = os.path.join(logging_dir_path,
                            "pyblazing." + str(ralId) + ".log")
    ...
    cio.blazingSetAllocatorCaller(
        allocator.encode(), initial_pool_size, config_options)
...
def generateGraphs(
    ...
    try:
        graph = cio.runGenerateGraphCaller(
                        ...
                        accessToken,
                        config_options)
        graph.set_input_and_output_caches(
            worker.input_cache, worker.output_cache)
...
def collectPartitionsPerformPartition(
    masterIndex, nodes, ctxToken, input,
    partition_keys_mapping, df_schema, by, i
):
    import dask.distributed
...
def parseHiveMetadata(curr_table, uri_values):
    ...
    for file_index, uri_value in enumerate(uri_values):
        for index, [col_name, col_value_id] in enumerate(uri_value):
            ...
            if dtypes[col_index] == np.dtype("object"):
                ...
                np_col_value = np.fromstring(
                    col_value_id, dtypes[col_index], sep=" ")[
                    0
                ]
    ...
    col2 = pandas.Series(
        minmax_metadata_table[2 * index + 1], 
        dtype=np.int32, name=names[2 * index + 1]
    )
    final_names.append(names[2 * index])
    final_names.append(names[2 * index + 1])
    series.append(col1)
    ...
    frame = OrderedDict((key, value) for (key, value) in zip(
        final_names, series))
...
def adjust_due_missing_rowgroups(metadata, files):
    ...
    for ind in missing_file_inds:
        ...
        metadata["file_handle_index"][mask] = (
            metadata["file_handle_index"][mask] - 1)
    ...
    def getSlices(self, numSlices):
        ...
        if self.files is None:
            for i in range(0, numSlices):
                nodeFilesList.append(BlazingTable(
                    self.name, self.input, self.fileType))
            return nodeFilesList
...
class BlazingContext(object):
    ...
    def __init__(
        ...
        if (not "BLAZ_HOST_MEM_CONSUMPTION_THRESHOLD".encode()
                in self.config_options):
            thresh = "BLAZ_HOST_MEM_CONSUMPTION_THRESHOLD".encode()
            self.config_options[thresh] = str(
                host_memory_quota
            ).encode()

        if dask_client is not None:
            distributed_initialize_server_directory(
                self.dask_client, logging_dir_path)
            distributed_initialize_server_directory(
                self.dask_client, cache_dir_path)
            ...
            if "BLAZ_HOST_MEM_CONSUMPTION_THRESHOLD" in config_options:
                host_memory_quota = float(
                    self.config_options[
                        "BLAZ_HOST_MEM_CONSUMPTION_THRESHOLD".encode()]
                )
            ...
            # need to initialize this logging independently, 
            # in case its set as a relative path
            # and the location from where the python 
            # script is running is different
            ...
            logging.basicConfig(
                filename=filename, format=FORMAT, level=logging.INFO)
        else:
            initialize_server_directory(logging_dir_path)
    ...
    def create_table(self, table_name, input, **kwargs):
        ...
        if user_partitions is not None and not isinstance(
        ...
        if isinstance(input, cudf.DataFrame):
            ...
            parsedSchema = self._parseSchema(
                input, file_format_hint, kwargs,
                extra_columns, ignore_missing_paths
            )
            ...
            if parsedSchema["file_type"] == DataType.PARQUET:
                ...
                file_groups = (
                    table.metadata.groupby("file_handle_index")._grouped())
    ...
    def _parseSchema(
        self, input, file_format_hint, 
        kwargs, extra_columns, ignore_missing_paths
    ):
        if self.dask_client:
    ...
    def _parseMetadata(self, file_format_hint, 
                       currentTableNodes, schema, kwargs):
        if self.dask_client:
            dask_futures = []
            ...
            return cio.parseMetadataCaller(
                files, currentTableNodes[0].offset,
                schema, file_format_hint, kwargs
            )
    ...
    def _sliceRowGroups(self, numSlices, files, uri_values, row_groups_ids):
        ...
        for i in range(0, numSlices):
            ...
            for ind, file_index in enumerate(file_indexes_for_slice):
                ...
                sliced_rowgroup_ids[-1].append(
                    flattened_rowgroup_ids[ind + startIndex])
    ...
    def _optimize_skip_data_getSlices(
        ...
        if not skipdata_analysis_fail:
            ...
            if (
                ...
                grouped = (
                    file_and_rowgroup_indices.groupby("file_handle_index"))
                for group_id in grouped.groups:
                    row_indices = grouped.groups[group_id].values.tolist()
    ...
    def sql(
        ...
        # algebra = modifyAlgebraForDataframesWithOnlyWantedColumns(algebra,
        #  relational_algebra_steps,self.tables)
            ...
            if (
                ...
                if query_table.has_metadata():
                    ...
                    if single_gpu:
                        ...
                        currentTableNodes = query_table.getSlices(
                            len(self.nodes))
        ...
        if self.dask_client is None:
            ...
            if single_gpu is True:
                # the following is wrapped in an array because .sql
                # expects to return
